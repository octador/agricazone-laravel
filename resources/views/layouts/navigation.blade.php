<nav x-data="{ open: false }" class="bg-customGreen-500 shadow" style="background-color:#36A793">
    ...
    <div class="max-w-10xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('stocks.create')" :active="request()->routeIs('stocks.create')" class="text-white dark:text-white">
                        <div class="flex items-center space-x-2">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('collections.index')" :active="request()->routeIs('stocks.create')" class="text-white dark:text-white">
                        <div class="flex items-center space-x-2">

                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="white" class="bi bi-geo" viewBox="0 0 16 16">
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('logout')" class="text-36A793 hover:text-white dark:text-36A793 dark:hover:text-white" onclick="event.preventDefault(); this.closest('form').submit();">
                                {{ __('Déconnecter') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        @if(auth()->user()->role_id == 3)
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('reservations.index')" :active="request()->routeIs('dashboard')" class="text-white dark:text-white flex items-center space-x-2">
                <img src="{{ asset('images/panier.svg') }}" alt="Panier" class="h-8 w-8">
        ...
        @if(auth()->user()->role_id == 2)
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('reservations.search', auth()->user()->id)" :active="request()->routeIs('dashboard')" class="text-white dark:text-white flex items-center space-x-2">
                <img src="{{ asset('images/stock.svg') }}" alt="Panier" class="h-8 w-8">
                <p>Mes Stock</p>
            </x-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <x-responsive-nav-link :href="route('logout')" class="text-white dark:text-white" onclick="event.preventDefault(); this.closest('form').submit();">
                        {{ __('Déconnexion') }}
                    </x-responsive-nav-link>
                </form>
            </div>
